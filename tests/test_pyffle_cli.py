...
# First party modules
from pyffle import pyffle_cli


def test_author() -> None:
    """Test function to assert correct author name."""
    runner = CliRunner()
    result = runner.invoke(pyffle_cli.author)
...
def test_author_verbose() -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(pyffle_cli.author, ["--verbose"])
    assert result.exit_code == 0
    assert (
...
def test_author_help() -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(pyffle_cli.author, ["--help"])
    assert result.exit_code == 0
